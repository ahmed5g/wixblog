...
public class PostService {
    ...
    public PostResponse createPost (CreatePostRequest request, Long authorId) {
        ...
        Category category = categoryService.getCategoryBySlug(request.getCategorySlug());
        ...
        tagService.processTagInput(request.getTags()).forEach(post::addTag);
    ...
    public Page<PostResponse> getPostsByCategory (String categorySlug, Pageable pageable) {
        Page<Post> posts = postRepository.findByCategorySlugAndStatus(
                categorySlug,
    ...
    @Transactional(readOnly = true)
    public Page<PostResponse> getPostsByCategoryAndTags (String categorySlug,
                                                 Set<String> tagSlugs,
                                                 Pageable pageable) {
        Page<Post> posts = postRepository.findByCategorySlugAndStatusAndTagsSlugIn(
                categorySlug,
                PostStatus.PUBLISHED,
                tagSlugs.stream().toList(),
                pageable
                                                               );
        return posts.map(postMapper::toResponse);
    }
    ...
    public Page<PostResponse> getPostsByTag (String tagSlug, Pageable pageable) {
        Page<Post> posts= postRepository.findByTagSlugAndStatus(tagSlug, PostStatus.PUBLISHED,
                                                                pageable);;
        return posts.map(postMapper::toResponse);
    }

    @Transactional
    public PostResponse updatePostTags (Long postId, Set<String> newTagNames) {
        Post post = postRepository.findById(postId)
                .orElseThrow(() -> new ResourceNotFoundException("Post not found"));
        post.getTags().clear();
        Set<Tag> newTags = tagService.processTagInput(newTagNames);
